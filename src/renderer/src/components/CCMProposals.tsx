...
export default function CCMProposals({ proposals, onResolve }: Props): React.ReactElement | null {
  ...
  return (
    <div className="flex flex-col gap-2 px-4 py-3 border-t border-white/5">
      <p className="text-white/50 text-[10px] uppercase tracking-wider">Memory Proposals</p>
      <div role="list" aria-label="Memory proposals">
        {proposals.map(p => (
          <div
            key={p.id}
            role="listitem"
            className="flex items-start gap-2 p-2.5 rounded-xl bg-white/5 border border-violet-400/10 mb-2 last:mb-0"
          >
            ...
            <div className="flex gap-1 shrink-0">
              <button
                ...
                aria-label={`Accept memory proposal: ${p.proposedKey}`}
              ...
              <button
                ...
                disabled={resolving === p.id}
                aria-label={`Reject memory proposal: ${p.proposedKey}`}
